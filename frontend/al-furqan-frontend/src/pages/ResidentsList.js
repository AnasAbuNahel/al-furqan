...
const ResidentsList = () => {
  ...
  const fetchResidents = async () => {
    try {
      ...
      const res = await axios.get('https://al-furqan-anas-new.onrender.com/api/residents', {
  ...
  const handleSave = async () => {
    const token = localStorage.getItem('token');
    try {
      await axios.put(`https://al-furqan-anas-new.onrender.com/api/residents/${formData.id}`, formData, {
        headers: { Authorization: `Bearer ${token}` },
      });
  ...
  const handleDelete = async (id) => {
    const token = localStorage.getItem('token');
    try {
      await axios.delete(`https://al-furqan-anas-new.onrender.com/api/residents/${id}`, {
        headers: { Authorization: `Bearer ${token}` },
      });
  ...
  const importFromExcel = (e) => {
    ...
    const token = localStorage.getItem('token');
    axios
      .post('https://al-furqan-anas-new.onrender.com/api/residents/import', data, {
        headers: { Authorization: `Bearer ${token}` },
      })
      .then(() => {
